...
def karakter_anahtarini_bul(karakter):
    ozel_karakterler = {
        ...
        '"': 'ozel_tirnak', "'": 'ozel_tektirnak', '[': 'ozel_koseli_ac', 
        ...
        '@': 'ozel_at', '$': 'ozel_dolar', '&': 'ozel_ampersand'
    }
    ...
    base = tr_map.get(karakter, karakter.lower())
    
    if karakter.isdigit(): return f"rakam_{karakter}"
    elif karakter.isupper(): return f"buyuk_{base}"
    elif karakter.islower() or karakter in tr_map: return f"kucuk_{base}"
    return None
    ...
    if anahtar and anahtar in harfler:
        ...
        for i in range(harf_resmi.size[0]):
            for j in range(harf_resmi.size[1]):
                r, g, b, a = pixels[i, j]
                # Daha esnek piksel kontrolü: Siyah tonlu veya yüksek opaklıkta olanları renklendir
                if (r < 180 and g < 180 and b < 180) or a > 150:
