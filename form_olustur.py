...
def create_form(filename="form.pdf", repetition=3, test_mode=False):
    ...
    for page in range(num_pages):
        ...
        for sec in range(2): 
            ...
            # Markerlar
            for p in range(4):
                safe_m_id = m_id % 100  # FIX: DICT_4X4_100 için mod 100! 
...
if __name__ == "__main__":
    # Boş Formlar
    print("\n=== BOŞ FORMLAR OLUŞTURULUYOR ===")
    create_form("form_1x_BOS.pdf", repetition=1, test_mode=False)
    create_form("form_2x_BOS.pdf", repetition=2, test_mode=False)
    create_form("form_3x_BOS.pdf", repetition=3, test_mode=False)
    create_form("form_5x_BOS.pdf", repetition=5, test_mode=False)
    create_form("form_10x_BOS.pdf", repetition=10, test_mode=False)
    
    # Dolu Formlar (Örnek)
    print("\n=== ÖRNEK FORMLAR OLUŞTURULUYOR ===")
    create_form("form_1x_DOLU.pdf", repetition=1, test_mode=True)
    create_form("form_2x_DOLU.pdf", repetition=2, test_mode=True)
    create_form("form_3x_DOLU.pdf", repetition=3, test_mode=True)
    create_form("form_5x_DOLU.pdf", repetition=5, test_mode=True)
    create_form("form_10x_DOLU.pdf", repetition=10, test_mode=True)
    
    print("\n✅ TÜM FORMLAR OLUŞTURULDU!")
    print("1x → 1 sayfa (107 karakter)")
    print("2x → 2 sayfa (214 karakter)")
    print("3x → 3 sayfa (321 karakter)")
    print("5x → 5 sayfa (535 karakter)")
    print("10x → 9 sayfa (1070 karakter)")  # ← KRİTİK: 10x için 9 sayfa!